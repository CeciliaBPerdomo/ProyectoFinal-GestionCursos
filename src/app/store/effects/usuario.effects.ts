    ...
    this.actions$.pipe(
      ...
      exhaustMap(() =>
        this.usuarioService.getUsuarios().pipe(
          ...
          catchError((error) => of(UsuarioActions.loadUsuariosFailure({ error: error.message })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.usuarioService.getUsuariosPorRol(action.rol as any).pipe(
          map((usuarios) => UsuarioActions.loadUsuariosByRolSuccess({ usuarios, rol: action.rol })),
          catchError((error) => of(UsuarioActions.loadUsuariosByRolFailure({ 
            error: error.message, 
            rol: action.rol 
          })))
        )
    ...
    this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.usuarioService.getUsuarioPorId(action.id).pipe(
          map((usuario) => {
            if (usuario) {
              return UsuarioActions.loadUsuarioSuccess({ usuario });
            } else {
              return UsuarioActions.loadUsuarioFailure({ error: 'Usuario no encontrado' });
            }
          }),
          catchError((error) => of(UsuarioActions.loadUsuarioFailure({ error: error.message })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.usuarioService.agregarUsuario(action.usuario).pipe(
          ...
          catchError((error) => of(UsuarioActions.addUsuarioFailure({ error: error.message })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.usuarioService.actualizarUsuario(action.usuario).pipe(
          ...
          catchError((error) => of(UsuarioActions.updateUsuarioFailure({ error: error.message })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.usuarioService.eliminarUsuario(action.id).pipe(
          ...
          catchError((error) => of(UsuarioActions.deleteUsuarioFailure({ error: error.message })))
        )
      )
    )
  );
